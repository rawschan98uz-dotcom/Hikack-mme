...
const funnelStats = computed(() => {
  if (!data.value) return { total: 0, booked: 0, attended: 0, rejected: 0, attendedRate: 0, rejectedRate: 0 };
  const p = data.value.pipeline;
  const booked = p['trial_booked'] ?? 0;
  const attended = p['attended'] ?? 0;
  const rejected = p['rejected'] ?? 0;
  const total = booked + attended + rejected;
  const attendedRate = (booked + attended) > 0 ? Math.round((attended / (booked + attended)) * 100) : 0;
  const rejectedRate = total > 0 ? Math.round((rejected / total) * 100) : 0;
  return { total, booked, attended, rejected, attendedRate, rejectedRate };
});
...
async function exportCsv() {
  ...
  try {
    ...
    const res = await client.get<ApiEnvelope<ConversionData>>('/reports/conversion', { params });
    const csvRows = [
      ['ID', 'ФИО (Full Name)', 'Телефон (Phone)', 'Текущий этап (Stage)', 'Записан на пробный', 'Был на уроке', 'Отказ/Архив', 'Дата создания'],
    ];
  ...
  <div class="space-y-4">
    ...
    <template v-else>
      ...
      <div v-if="funnelStats.total > 0" class="rounded-xl border border-fb-line bg-fb-card p-5 shadow-sm">
        <div class="flex items-center justify-between text-xs font-medium text-fb-secondary mb-3">
          <span>Воронка движения лидов</span>
          <span>Конверсия в урок: <strong class="text-fb-blue">{{ funnelStats.attendedRate }}%</strong></span>
        </div>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <div class="relative flex flex-col justify-between rounded-lg border border-gray-200 bg-gray-50/60 p-3.5">
            <div class="flex items-center justify-between">
              <span class="text-xs font-semibold uppercase tracking-wider text-gray-700">3. Отказ / Архив</span>
